...
title_text = font.render("Title Slide", True, BLACK)
...
while True:
    ...
    if ret:
        ...
        if key==ord('a'):
                 ...
                 filename2 = 'screen_record_{}.avi'.format(datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
                 screen_size = (1920, 1080)
                 out = cv2.VideoWriter(filename2, fourcc, 20.0, screen_size)
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        filename2 = 'screen_record_{}.avi'.format(datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
        screen_size = (1920, 1080)
        out = cv2.VideoWriter(filename2, fourcc, 20.0, screen_size)
        ...
        while flag==DSA:           
                 ...
                 cap1 = cv2.VideoCapture(filename2)
                 ...
                 if ret:
                   ...
                   if key==ord('a'):
                     show_notification("Already in DSA mode")  
                     flag=DSA                              
                   if key==ord('m'):
                     show_notification("Roadmap mode")
                     flag =Roadmap
                   ...
                   if key == ord('d'):
                     if not Recording:
                        ...
                        if elapsed_time > 0.1:
                         out.write(np.array(ImageGrab.grab(bbox=(0, 0, 1920, 1080))).astype(np.uint8))
                         start_time = time.time()
                   if keyboard.is_pressed('l'):
                     ret, frame2 = cap1.read()
                     show_video1 = True
                     cv2.imshow('angio sim', frame2) 
                     # Read the screen
                   img = ImageGrab.grab(bbox=(0, 0, 1920, 1080))
                   frame1 = np.array(img)
                   frame2 = cv2.cvtColor(frame1, cv2.COLOR_BGR2RGB)
                   if show_video1:
                     ret, frame3 = cap1.read()
                     if ret:
                         cv2.imshow('angio sim', frame3)
                     else:
                         show_video1 = False
                         # If recording is enabled, write the frame to the video file
                   if Recording:
                     out.write(frame2)
    
                     # If the 'l' key is pressed and show_video is True, display the video in the 'angio sim' window

        
                     # If the 'l' key is released, stop displaying the video
                   if show_video1 and not keyboard.is_pressed('l'):
                     show_video1 = False
    
                     # Check if the 'd' key is released to stop recording the screen
                   if Recording and not keyboard.is_pressed('d'):
                      Recording = False
                      out.release()
