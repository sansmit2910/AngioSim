        ...
        while flag==DSA:           
                 ...
                 if ret:
                     ...
                     if key == ord('l'):
                            ...
                            frame_count = 0
                            delay = int(1000 / fps)  # calculate the delay between frames based on the desired frame rate
                            while show_video1:
                                # Read the next frame from the saved video file
                                ret, frame1 = saved_video.read()
                                if ret:
                                    frame_count += 1
                                    cv2.imshow('angio sim', frame1)
                                    key = cv2.waitKey(delay)  # wait for the appropriate amount of time before displaying the next frame
                                if key == 27:
                                    exit()
                                elif key == ord('l'):
                                        break 
