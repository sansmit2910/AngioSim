...
while True:
    ...
    if ret:
        ...
        filename2 = 'screen_record_DSA_{}.avi'.format(datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
        # Set the screen size
        screen_size = (1920, 1080)
        fps = 2
        ...
        while flag==DSA:           
                 ...
                 if ret:
                     ...
                     while show_video1:
                               ...
                               ret, frame4 = saved_video.read()
                               if ret:
                                   # Display the current frame
                                   cv2.imshow('angio sim', frame4)
                                   frame_count += 1

                                   # Delay for 33ms to achieve 30fps
                               key = cv2.waitKey(33)

                               # Exit video playback if 'l' key is pressed again
